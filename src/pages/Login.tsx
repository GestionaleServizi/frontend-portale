...
import { useAuth } from "../hooks/useAuth"; // 👈 usa hook centralizzato
...
export default function Login() {
  ...
  const { login } = useAuth(); // 👈 importata logica login

  const handleLogin = async () => {
    try {
      const user = await login(email, password); // 👈 usa login di useAuth
